...
def test_sections_present():
    ...
    assert sum(1 for ln in page.splitlines()
               if ln.startswith("- **")) in (3, 4)              # 1: framing bullets
    ...
    assert "## Figures" in page                                 # 4: upload list
...
def test_marks_preserved_and_diagnostics_dropped():
    ...
    assert "(0.76 @512)" not in page    # ...with the rerun diagnostic dropped
    assert "| 0.66* |" in page and "| +0.02* |" in page
    assert "| 0.94† |" in page and "| +0.17† |" in page
    assert "| 0.64‡ |" in page
