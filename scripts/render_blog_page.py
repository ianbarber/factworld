...
of that artifact:
  ...
  1. title + framing bullets (what FactWorld measures, the two regimes in plain
     words, floors/marks, repo + full report links)
  2. the simplified five-column results table for the CURRENT roster — values
     and cleanliness marks parsed from the headline table, never hardcoded;
     the escalated reruns' parenthesised diagnostics are dropped for the blog
     (the canonical value + marks stay); the floor rows are kept
  3. a one-line marks explainer + the two floor definitions, one line each
  4. the figure upload list with one-line captions and alt text
  5. a "Last updated" footer (date from results.md's generated stamp, roster
     size, repo link, the three refresh commands)
...
FRAMING = [
    ...
    "independently and then composed into a single two-hop question. The "
    "components are cheap for every model; the composition is where they "
    "diverge.",
    ...
    f"intervals: [full results]({REPORT_URL}).",
]

MARKS_LINE = (
    "Marks: `*` the model cannot fully disable reasoning; `†` visible working "
    "leaked onto a supposedly instant attempt — read it as a soft upper bound; "
    "`‡` the provider did not enforce the token cap, so token comparisons are "
    "off; `⊘ >budget` the token budget ran out before an answer — not "
    "measurable at that budget, which is different from a zero.")
...
# (filename, one-line caption, alt text) — the blog upload list. Alt text stays
# qualitative on purpose: the numbers live in the table above, parsed fresh
# from results.md on every render.
FIGURES = [
    ("fig_zero_budget.png",
     ...
     "model.",
     "Grouped bar chart of the model roster with reasoning off. For each model, "
     "bars show state tracking at length 16, the composed two-hop task at "
     "lengths 16 and 64, and a test-retest replicate, with Wilson 95% error "
     "bars and the composition gap annotated."),
    ("fig_profiles.png",
     ...
     "Small-multiples panel, one chart per roster model. Horizontal bars give "
     "each model's normalized position on six axes — state tracking, composed "
     "score, composition gap (inverted), pointer-chase depth 128, S5 at length "
     "256, and completion tokens on the matched S5 cell (inverted) — with raw "
     "values printed beside the bars; unmeasurable cells are gaps, not zeros."),
    ("fig_chain_nowrap.png",
     ...
     "Line chart of relaxed accuracy versus pointer-chase depth, 16 to 128 on "
     "a log scale, with reasoning enabled; one line per roster model, hollow "
     "markers where the token budget ran out before an answer."),
    ("fig_s5_horizon.png",
     "Permutation state with reasoning on: tracking five people across five "
     "jobs through up to 256 swap/cycle events.",
     "Line chart of relaxed accuracy versus permutation stream length, 16 to "
     "256 on a log scale, with reasoning enabled; one line per roster model, "
     "hollow markers where the token budget ran out before an answer."),
]

THREE_COMMANDS = (
    "register the model's slug, run `scripts/run_frontier_benchmark.py`, "
    "re-render with `scripts/render_benchmark.py`")
...
def footer_line(date: str, n_models: int) -> str:
    return (f"Last updated: {date} · {n_models} models · benchmark data and "
            f"methodology: [{REPO_LABEL}]({REPO_URL}) · refresh: three commands "
            f"({THREE_COMMANDS}).")
