...
`followups/parametric-recall/scale_tuned.py`. gdp_hybrid, mixed-density, 8000 steps, 2 seeds. Largest scales with a per-scale LR sweep + a ~70M ceiling point. Headline = **answer-only L64** (the internalized horizon wall; floored 0/3 across 5.7M→44.8M at fixed lr in scale.py). Floor = 0.20. Still floor here = not relieved by capacity OR LR-tuning within 3090 reach.
