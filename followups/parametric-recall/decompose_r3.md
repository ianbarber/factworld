...
`followups/parametric-recall/decompose_r3.py`. gdp_hybrid d256x4, 4000 steps, 5 seeds, CoT. Same fixed map + same non-abelian chains in both arms; only difference is whether the agent->a0 facts are rendered (`inctx`) or omitted (`param`). Free-running eval (model emits its own holder then value). Floor = 0.20.
