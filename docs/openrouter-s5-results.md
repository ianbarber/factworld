# FactWorld OpenRouter Model Grid — S₅ non-abelian state-tracking
...
Task: `s5_v1` — non-abelian role-permutation state-tracking (experimental). A sequence of
`swap`/`cycle_roles` events permutes which agent holds each of five roles. The query asks for the
role of a single agent at the end. The answer is one of five role tokens, so the random-guess
floor is **0.20**.
...
- **Format instructions are necessary but not sufficient.** Without the role-token instruction,
  ...
  *shape*, but they cannot track the running non-abelian permutation.
- **This matches the custom-trained learnability map.** The same S₅ task is solvable by a small
  recurrent model when trained with dense per-step process supervision (see
...
- The strongest composite model in the grid, Nemotron 3 Ultra (0.767 on `composite_copy_v1`), is
  not stronger than the others here — non-abelian state-tracking is a distinct bottleneck.
