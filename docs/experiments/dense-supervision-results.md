...
**Question:** is the `s5_v1` non-abelian state-tracking deficit movable for our locally trained
...
## Sparsity threshold (10-seed, gdp_hybrid)
...
**The deficit moves.** Dense K=1 supervision solves s5 in-distribution (1.00, 10/10 converge) where
...
near-dense threshold on the natural format — the mechanism is format-independent.

**Length extrapolation is the bimodal regime.** K=1 reaches L64=0.75±0.22 and (on 2 seeds) L128≈0.90 —
roughly 4–8× the trained length, with no target-length labels — but the variance is real. K=2's
...
## Inference-time scaling probe — does test-time compute loosen the threshold?
...
| K=1 @L128 (circuit formed, 8× train length) | 0.88–0.90 | — | 0.88–0.90 | — |

**Inference-time compute does not move the deficit in either direction.** Seeds that formed the
circuit are already greedy-optimal (voting is a no-op, even at 30 votes); seeds that didn't stay at
the floor no matter how many samples. The constraint is in **learnability (supervision density)**,
not in test-time compute.
...
**Headline:** both deficits (composition routing, and non-abelian state-tracking) resist test-time
compute. The composition deficit yields only to an *oracle-provided* intermediate; the s5 deficit yields
